        ...
        private void BuyInvoiceItemGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
                ...
                e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == BuyInvoiceItemGridView.Columns["SelectItemBtn"].Index)
                {
                    ChooseForm itemForm;
                    ExplicitArguments args = new ExplicitArguments();
                    args.SetArg("itemid", BuyInvoiceItemGridView.Rows[e.RowIndex].Cells["ItemId"].Value != DBNull.Value ? (int)BuyInvoiceItemGridView.Rows[e.RowIndex].Cells["ItemId"].Value : 0);
                    ExplicitArguments chooseArgs = new ExplicitArguments();
                    chooseArgs.SetArg("view", IOC.Container.GetInstance<ItemUiView>(args));
                    itemForm = IOC.Container.GetInstance<ChooseForm>(chooseArgs);
