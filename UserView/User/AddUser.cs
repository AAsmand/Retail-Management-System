...
namespace Project.Forms.User
{
    public partial class AddUser : Form
    {
        ...
        public AddUser(IUserBusiness userBusiness,int userId)
        {
            InitializeComponent();
            this.userBusiness = userBusiness;
            _userId = userId;
        ...
        private void RolesGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
                ...
                e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == RolesGridView.Columns["SelectRoleBtn"].Index)
                {
                    ...
                    args.SetArg("roleId", RolesGridView.Rows[e.RowIndex].Cells["RoleId"].Value != null ? (int)RolesGridView.Rows[e.RowIndex].Cells["RoleId"].Value : 0);
                    ExplicitArguments chooseArgs = new ExplicitArguments();
                    chooseArgs.SetArg("view", IOC.Container.GetInstance<RoleUiView>(args));
                    chooseRollForm = IOC.Container.GetInstance<ChooseForm>(chooseArgs);
